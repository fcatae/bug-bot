...
namespace BugBot
{
    public static class FormattableObject
    {
        ...
        public static string ToString(this JObject anObject, string aFormat)
        {
            StringBuilder sb = new StringBuilder();
            Type type = anObject.GetType();
            Regex reg = new Regex(@"({)([^}]+)(})", RegexOptions.IgnoreCase);
            ...
            foreach (Match m in mc)
            {
                ...
                JToken retrievedToken = null;
                //first try properties
                if (anObject.TryGetValue(toGet, out retrievedToken))
                {
                    string result = retrievedToken.ToString();
                    sb.Append(result);
                }
                else //didn't find a property with that name, so be gracious and put it back
                {
